...
def find_urls(s):
    urls = re.findall("((?:http|ftp|https):\/\/[\w_-]+(?:(?:\.[\w_-]+)+))(?:[\w.,@?^=%&:/~+#-]*[\w@?^=%&/~+#-])?", s)
    return(urls)
...
def grab_headlines():
    ...
    with open("opinion.html") as fp:
        soup = BeautifulSoup(fp, 'html.parser')
  
    mostread = soup.find_all('div', 'item-list')[-1]
    for thing in mostread:
        k = thing('a')
        for lines in k:
            headlines.append(lines.string)
    return headlines
...
def get_umsi_data():
    ...
    soups = []
    rs = []
    # base_url = "https://www.si.umich.edu/directory?field_person_firstname_value=&field_person_lastname_value=&rid=All&page=2"
    # for i in range(13):
    #     r = requests.get(base_url, headers={'User-Agent': 'SI_CLASS'})
    #     soup = BeautifulSoup(r.text, 'html.parser')
    #     directory = soup.find('div', class_='view-content')
    #     names = directory.find_all('div', class_="field-item even")
    #     for name in names: 
    #         for sub_heading in name.find_all('h2'):
    #             if "." not in sub_heading.text:
    #                 thename = sub_heading.text
    #                 allnames.append(thename)
    #     titles = directory.find_all('div', class_="field field-name-field-person-titles field-type-text field-label-hidden")
    #     for title in titles:
    #         tit = title.text
    #         alltitles.append(tit)
    # k = 0
    # for k in range(len(allnames)):
    #     umsi_titles[allnames[k]] = alltitles[k]
    # i = i + 1
    # print(umsi_titles)
    for i in range(13):
        ...
        r = requests.get(base_url, headers={'User-Agent': 'SI_CLASS'})
        rs.append(r.text)
        ##WHY THE FUCK is soup the same each time i run this? It's impossible! The request changes, the url changes.
        #HOW CAN THE SOUP VARIABLE POSSIBLY STAY THE SAME!!!!!?????
        soup = BeautifulSoup(r.text, 'html.parser')
        soups.append(soup)
        directory = soup.find('div', class_='view-content')
        soup = ""
