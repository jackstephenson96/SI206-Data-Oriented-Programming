...
def get_user_tweets(user):
	if user in CACHE_DICTION:
		...
		print("fetching")
		uh = api.user_timeline(screen_name = user, count = 20)
		returnies = []
		for status in uh:
			returnies.append(status)
		CACHE_DICTION[user] = returnies
...
users = []
tweets = []
for tweeter in umich_tweets:
	#users
	user = {}
	user["user_id"] = tweeter["user"]["id_str"]
	user["screen_name"] = tweeter["user"]["screen_name"]
	user["num_favs"] = tweeter["user"]["favourites_count"]
	user["description"] = tweeter["user"]["description"]
	if user not in users:
		users.append(user)
	#tweets
	twt = {}
	twt["tweet_id"] = tweeter["id_str"]
	twt["text"] = tweeter["text"]
	twt["user_posted"] = tweeter["user"]["id_str"]
	twt["time_posted"] = tweeter["created_at"]
	twt["retweets"] = tweeter["retweet_count"]
	if twt not in tweets:
		tweets.append(twt)

	#mentioned users
	
	if type(tweeter["entities"]["user_mentions"]) == list:
		for i in tweeter["entities"]["user_mentions"]:
			user = {}
			user["user_id"] = i["id_str"]
			user["screen_name"] = i["screen_name"]
			ku = api.get_user(i["id_str"])
			user["num_favs"] = ku["favourites_count"]
			user["description"] = ku["description"]
			if user not in users:
				users.append(user)
...
cur.execute('CREATE TABLE Users (user_id TEXT, screen_name TEXT, num_favs TIMESTAMP, description TEXT)')
cur.execute('DROP TABLE IF EXISTS Tweets')
cur.execute('CREATE TABLE Tweets (tweet_id TEXT, text TEXT, user_posted TIMESTAMP, time_posted TEXT, retweets INTEGER)')

for u in users:
	cur.execute('INSERT INTO Users (user_id, screen_name, num_favs, description) VALUES (?, ?, ?, ?)',
    	(u["user_id"], u["screen_name"], u["num_favs"], u["description"]))
for t in tweets:
	cur.execute('INSERT INTO Tweets (tweet_id, text, user_posted, time_posted, retweets) VALUES (?, ?, ?, ?, ?)',
    	(t["tweet_id"], t["text"], t["user_posted"], t["time_posted"], t["retweets"]))
conn.commit()
...
joined_data = []
cur.execute("SELECT Users.screen_name, Tweets.text FROM Users INNER JOIN Tweets ON Tweets.user_posted = Users.user_id")
for tup in cur:
...
joined_data2 = []
cur.execute("SELECT Users.screen_name, Tweets.text FROM Users INNER JOIN Tweets ON Tweets.user_posted = Users.user_id ORDER BY Tweets.retweets desc")
for sortedtup in cur:
	joined_data2.append(sortedtup)
cur.close()
